...
authorize('access_admin_panel', '../index.php', false);

header('Content-Type: text/html; charset=utf-8');

// --- Verificaciones de instalación y base de datos ---
try {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    $allowed_tabs = ['configuracion','correos_autorizados','servidores','otros','logs','asignaciones'];
    $current_tab = filter_var($_POST['current_tab'] ?? 'configuracion', FILTER_SANITIZE_STRING);
    ...
    if ($update_servers_only || $current_tab == 'servidores') {
        foreach ($email_servers_data as $server) {
            ...
            $imap_server = filter_var($_POST["imap_server_$server_id"] ?? $server['imap_server'], FILTER_SANITIZE_STRING);
            $imap_port = filter_var($_POST["imap_port_$server_id"] ?? $server['imap_port'], FILTER_VALIDATE_INT);
            $imap_user = filter_var($_POST["imap_user_$server_id"] ?? $server['imap_user'], FILTER_SANITIZE_STRING);
            
            if (isset($_POST["imap_password_$server_id"])) {
                if ($_POST["imap_password_$server_id"] !== '**********' && $_POST["imap_password_$server_id"] !== '') {
                    $imap_password = filter_var($_POST["imap_password_$server_id"], FILTER_SANITIZE_STRING);
                } else {
            ...
            if ($imap_port === false || $imap_port < 1 || $imap_port > 65535) {
                $imap_port = 993;
            }
    ...
    if (!$update_servers_only) {
        ...
        try {
            ...
            $fresh_settings = ConfigService::getInstance()->getAll();
    ...
    AuditLogger::log($_SESSION['user_id'] ?? null, 'config_update');
        ...
        <div class="tab-pane fade" id="logs" role="tabpanel">
            <div class="admin-card">
                <div class="admin-card-header">
                    ...
                    <div class="action-buttons-group">
                        <a href="export_logs.php" class="btn-admin btn-info-admin">
